<?php
    /**
     * @package POWr Holiday Countdown
     * @version 1.6.4
     */
    /*
    Plugin Name: POWr Holiday Countdown
    Plugin URI: https://www.powr.io/plugins/holiday-countdown
    Description: Count down to any holiday, birthday, or special event!  Drop the widget anywhere in your theme. Or use the POWr icon in your WP text editor to add to a page or post. Edit on your live page by clicking the settings icon. More plugins and tutorials at POWr.io.
    Author: POWr.io
    Version: 1.6.4
    ...
    */
    ...
    //////////////////////////////////////////////////////////////////////////////////////////////////
    ///////////////////////////////////////Create Holiday Countdown widget/////////////////////////////////
    class Powr_Holiday_Countdown extends WP_Widget{
      public $plugin;
      //Create the widget
      public function __construct(){
        parent::__construct( 'powr_holiday_countdown',
                             __( 'POWr Holiday Countdown' ),
                             array( 'description' => __( 'Holiday Countdown by POWr.io') )
        );
      ...
      public function widget( $args, $instance ){
        ...
        ?>
        <div class='widget powr-holiday-countdown' label='<?php echo $label; ?>'></div>
        <?php
      }
      ...
      public function form( $instance ){
        ...
        ?>
        <p>
          To edit, visit your live webpage and click the gears icon on your Holiday Countdown.
        </p>
        <p>
    ...
    function register_powr_holiday_countdown() {
      register_widget( 'Powr_Holiday_Countdown' );
    }
    //Use widgets_init action hook to execute custom function
    add_action( 'widgets_init', 'register_powr_holiday_countdown' );
    //Create short codes for adding plugins anywhere:
    function powr_holiday_countdown_shortcode( $atts ){
      if(isset($atts['id'])){
        $id = $atts['id'];
      	return "<div class='powr-holiday-countdown' id='$id'></div>";
      }else if(isset($atts['label'])){
        $label = $atts['label'];
		    return "<div class='powr-holiday-countdown' label='$label'></div>";
      }else{
      	"<div class='powr-holiday-countdown'></div>";
      }
    }
    add_shortcode( 'powr-holiday-countdown', 'powr_holiday_countdown_shortcode' );
    ...
    //ADD MENUS
    add_action( 'admin_menu', 'powr_holiday_countdown_menu' );
    function powr_holiday_countdown_menu() {
      add_menu_page( 'POWr Holiday Countdown', 'POWr Holiday Countdown', 'manage_options', 'powr-holiday-countdown-settings', 'powr_holiday_countdown_options', plugins_url('/powr-icon.png',__FILE__));
    }
    function powr_holiday_countdown_options() {
      if(powr_local_mode()){//Determine JS url:
        $redirect_url = 'https://localhost:3000/wp-create/holiday-countdown';
      }else{
        $redirect_url = 'https://www.powr.io/wp-create/holiday-countdown';
      }
    ...
    register_activation_hook(__FILE__, 'powr_holiday_countdown_plugin_activate');
      add_action('admin_init', 'powr_holiday_countdown_plugin_redirect');

      function powr_holiday_countdown_plugin_activate() {
      add_option('powr_holiday_countdown_plugin_do_activation_redirect', true);
      }
      ...
      function powr_holiday_countdown_plugin_redirect() {
      if (get_option('powr_holiday_countdown_plugin_do_activation_redirect', false)) {
          delete_option('powr_holiday_countdown_plugin_do_activation_redirect');
          if(!isset($_GET['activate-multi']))
          {
            wp_redirect( get_admin_url().'?platform=wordpress&page=powr-holiday-countdown-settings&' );
          }
       }
      }
